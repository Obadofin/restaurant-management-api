...
const createOrder = async () => {
  const catRes = await request(app).post("/api/categories").set("Authorization", `Bearer ${adminToken}`).send({ name: "Mains", description: "Main dishes" });
  const categoryId = catRes.body.data._id;

  const menuRes = await request(app).post("/api/menu").set("Authorization", `Bearer ${adminToken}`).send({ name: "Jollof Rice", price: 1500, category: categoryId, isAvailable: true });
...
describe("POST /api/payments/process", () => {
  it("returns 401 when not authenticated", async () => {
    const res = await request(app).post("/api/payments/process").send({ amount: 25.99, paymentMethod: "card" });
    expect(res.statusCode).toBe(401);
  });

  it("processes a standalone payment and returns success with a transactionId", async () => {
    const res = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 25.99, paymentMethod: "card" });
  ...
  it("accepts cash as a payment method", async () => {
    const res = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 10, paymentMethod: "cash" });
  ...
  it("generates unique transactionIds for consecutive payments", async () => {
    const res1 = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 10, paymentMethod: "card" });

    const res2 = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 20, paymentMethod: "cash" });
  ...
  it("uses the order totalPrice when an orderId is provided", async () => {
    ...
    const res = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ order: orderId, paymentMethod: "card" });
  ...
  it("returns 404 when the provided orderId does not exist", async () => {
    const res = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ order: "000000000000000000000000", paymentMethod: "card" });

    expect(res.statusCode).toBe(404);
  });

  it("returns 400 when amount is missing and no orderId provided", async () => {
    const res = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ paymentMethod: "card" });

    expect(res.statusCode).toBe(400);
  });

  it("returns 400 for an invalid payment method", async () => {
    const res = await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 10, paymentMethod: "crypto" });
...
describe("GET /api/payments/me", () => {
  ...
  it("returns only the authenticated user's payments", async () => {
    await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 10, paymentMethod: "card" });

    await request(app).post("/api/payments/process").set("Authorization", `Bearer ${customerToken}`).send({ amount: 20, paymentMethod: "cash" });

    await request(app).post("/api/payments/process").set("Authorization", `Bearer ${staffToken}`).send({ amount: 30, paymentMethod: "card" });
...
describe("GET /api/payments", () => {
  ...
  it("admin can retrieve all payments", async () => {
    ...
    await request(app).post("/api/payments/process").set("Authorization", `Bearer ${staffToken}`).send({ amount: 20, paymentMethod: "cash" });
